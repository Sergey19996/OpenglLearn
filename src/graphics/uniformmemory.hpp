...
namespace UBO {
	...
	typedef struct Element {
		...
		Element(Type type = Type::SCALAR) : type(type), length(0), list(0) {
			...
			{
			case Type::SCALAR:
				baseAlign = N; break;   // 4 байта для выравнивания 
			case Type::VEC2:
				baseAlign = 2 * N; break; // 8 для vec2
			case Type::VEC3:
			case Type::VEC4:
				baseAlign = 4 * N; break; // 16 для vec3 и vec4
	...
	Element newStruct(std::vector<Element> subElements) {
		...
		//base alignment is largest if its subelements
		if (subElements.size()) {
	...
	public:
		...
		void writeArrayContainer(T* container, unsigned int n) {
			for (int i = 0; i < n; i++) {
				writeElement<V>(&container->operator[](i)); // container [] translate to container + i
			}
		}

		void advanceCursor(unsigned int n) {
			//skip number of element
			for (int i = 0; i < n; i++) {
				Element element = getNextElement();
				offset = roundUpPow2(offset, element.alignPow2());
				if (poppedOffset) {
		...
		void advanceArray(unsigned int noElements) {
			...
			if (currentElement->type == Type::STRUCT) {
				...
				if (currentElement->type != Type::ARRAY) {
					//did not find an array(reached pribitive)
					return;
				}
